...
class Highlighter:
    ...
    def highlight_helper(self, selected_id, marker_id_str):
        self.highlight_marker(highlight=selected_id)
        self.highlighted[marker_id_str]['highlighted'] = True

    def dehighlight_helper(self, selected_id, marker_id_str):
        self.highlight_marker(dehighlight=selected_id)
        self.highlighted[marker_id_str]['highlighted'] = False

    def highlighter_picked_handler(self, selected_id, from_picker=None, from_explorer=None, from_gaps=None,
                                   selected=None, deselected=None):
        ...
        if from_picker:
            ...
            if not self.highlighted[marker_id_str]['highlighted']:
                self.highlight_helper(selected_id, marker_id_str)
                ...
                self.dehighlight_helper(selected_id, marker_id_str)
            ...
            # and has selection model that sends both seleted and deselected items
            if selected:
                # this statement prevents recursion
                if not self.highlighted[marker_id_str]['highlighted']:
                    self.highlight_helper(selected_id, marker_id_str)
                    self.mainwindow.gaps.clear_gap_table()
    ...
    def clear_highlighted(self):
        for mrk, val in self.highlighted.items():
            if val['highlighted']:
                ...
                self.highlight_marker(dehighlight=int(mrk))
